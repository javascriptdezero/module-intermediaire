...
const enonces = {
  ...
  definirEchec(enonce, valeurs) {
    ...
    message.innerHTML = `<p>▶︎ Votre code renvoie :</p><pre class="valeur">${valeurRecue || 'Rien (undefined)'}</pre>
    <p>▶︎ Alors qu'il devrait renvoyer :</p><pre class="valeur">${valeurAttendue}</pre>
